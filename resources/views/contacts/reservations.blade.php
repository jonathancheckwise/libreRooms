...
@section('title', __('Reservations') . ' — ' . $contact->display_name())
...
<div class="max-w-5xl mx-auto py-8 px-4">
    ...
    <div style="margin-bottom:1rem">
        <a href="{{ route('contacts.index') }}" style="color:#6b7280;text-decoration:none;font-size:.9rem">← {{ __('Contacts') }}</a>
    </div>

    <h1 style="font-size:1.6rem;font-weight:700;margin:0 0 .25rem">{{ __('Reservations') }} — {{ $contact->display_name() }}</h1>
    <p style="color:#6b7280;margin:0 0 1.25rem">
        {{ $contact->email }}
        @if($contact->users()->exists())
            · <span style="color:#059669">{{ __('Registered user') }}</span>
        @else
            · <span style="color:#b45309">{{ __('External (no account)') }}</span>
        @endif
    </p>

    {{-- Filtre période --}}
    <form method="GET" action="{{ route('contacts.reservations', $contact) }}"
        ...
        @if($from || $to)
            <a href="{{ route('contacts.reservations', $contact) }}" class="btn btn-secondary">{{ __('Reset') }}</a>
        @endif
    </form>
