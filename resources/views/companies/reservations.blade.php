...
@section('title', __('Reservations') . ' — ' . $company->name)
...
<div class="max-w-5xl mx-auto py-8 px-4">
    ...
    <div style="margin-bottom:1rem">
        <a href="{{ route('companies.index') }}" style="color:#6b7280;text-decoration:none;font-size:.9rem">← {{ __('Companies') }}</a>
    </div>

    <h1 style="font-size:1.6rem;font-weight:700;margin:0 0 1.25rem">{{ __('Reservations') }} — {{ $company->name }}</h1>

    {{-- Filtre période --}}
    <form method="GET" action="{{ route('companies.reservations', $company) }}"
        ...
        @if($from || $to)
            <a href="{{ route('companies.reservations', $company) }}" class="btn btn-secondary">{{ __('Reset') }}</a>
        @endif
    </form>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:.9rem">
            <thead>
                <tr style="text-align:left;color:#6b7280;border-bottom:1px solid #e5e7eb">
                    ...
                    <th style="padding:.7rem 1rem">{{ __('Contact') }}</th>
            ...
            <tbody>
                @forelse($reservations as $r)
                    ...
                    <tr style="border-bottom:1px solid #f3f4f6">
                        ...
                        <td style="padding:.7rem 1rem">{{ $r->tenant?->display_name() ?? '—' }}</td>
